...
class Installer:
    ...
    @classmethod
    def _get_apt_complete_version(cls, name, version):
        temp_cmd = copy.copy(cls.cmd_apt_madison)
        temp_cmd.append(name)
        try:
            res = subprocess.run(temp_cmd, stdout=subprocess.PIPE, check=True)
    ...
    @classmethod
    def _get_apt_complete_package(cls, name, keywords):
    ...
    @classmethod
    def _install_one_gadget_by_version(cls, name, version, mappings=None):
        # get complete version, e.g. 18.03.1~ce-0~ubuntu
        complete_version = cls._get_apt_complete_version(name, version)
        if complete_version:
        ...
        try:
            if gpg_url:
                ...
                res = subprocess.run(
                    ...
                    check=True)
                subprocess.run(cls.cmd_apt_add_key, input=res.stdout, check=True)

            # add apt repository
            cmd_apt_add_repository = 'add-apt-repository\n' \
                                     '{repo_entry}'.format(
                                         repo_entry=repo_entry).split('\n')
            subprocess.run(cmd_apt_add_repository, check=True)
    ...
    @classmethod
    def _pull_images(cls, images, mappings=None):
        for image in images:
            cls._pull_image(image, mappings)

    @classmethod
    def _pull_image(cls, image, mappings=None):
        if mappings:
            mappings[image] = None
    ...
    def _pull_domestic_images(cls, images, ori_prefix,
                              new_prefix, mappings=None):
        for image in images:
            cls._pull_domestic_image(image, ori_prefix, new_prefix, mappings)
    ...
    @classmethod
    def _create_k8s_resources(cls, desc_file):
    ...
    @classmethod
    def _pull_quay_image(cls, image, domestic=False, mappings=None):
        if domestic:
            ...
            cls._pull_image(image, mappings=mappings)

    @classmethod
    def _pull_docker_image(cls, image, domestic=False, mappings=None):
        if domestic:
            cls._pull_domestic_image(
                image,
