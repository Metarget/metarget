...
下载 https://github.com/gabrtv/shocker 中的shocker.c 的poc，更改shocker.c中 .dockerinit 文件为 /etc/hosts
...
26 代表`/etc/passwd`的`mount_id` ，8是`file_handle`的`handle_bytes`，1是`handle_type`。后八个字节则是`f_handle`，他的大小等于`handle_bytes`（8个字节）。
...
利用上述宿主机文件/etc/shadow的`file_handl`细节，编写如下poc，然后在docker中执行。
...
https://man7.org/linux/man-pages/man7/capabilities.7.htm